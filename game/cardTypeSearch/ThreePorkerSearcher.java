...
public class ThreePorkerSearcher extends AbstractSearcher{
	public ThreePorkerSearcher(){
		m_nSearcherPieceNum = 3;
	}
	...
	public int searchBiggerPorkerThanMax(List<Integer> playerHandPorker, int nMaxHandPattern, int nMajorFaceValue, List<Integer> sumbitPorkerList){
		...
		if(false == bCheckResult){
			logger.error("ThreePorkerSearcher::searchBiggerPorkerThanMax playerHandPorker isnot Valid Error");
			return nErrorResult;
		}
		...
		if(null == biggerFaceValue){
			logger.error("ThreePorkerSearcher::searchBiggerPorkerThanMax biggerFaceValue null Error");
			return nErrorResult;
		}
		
		ParameterUtils parameter = new ParameterUtils();
		parameter.setNeedPorkerNum(this.getSearcherPieceNum());
		parameter.setCardTypeEnumValue(CardTypeEnum.CARD_TYPE_THREE_PIECE_VALUE);
		...
		if(null == smallerFaceValueList){
			logger.error("ThreePorkerSearcher::searchBiggerPorkerThanMax smallerFaceValueList null Error");
			return nErrorResult;
		}
